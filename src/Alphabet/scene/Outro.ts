...
export class Outro extends SceneBase {
	...
	async onInit() {
		...
		// 	).texture,
		// );
		this.mCloseBtn = new Button(tViewSheet.textures['big_close_btn.png']);
		...
		// this.mTureTrue = new PIXI.Sprite(
		// 	ViewerRscManager.Handle.getResource(this.name, 'outro_cha.png').texture,
		// );
		this.mTureTrue = new PIXI.Sprite(tViewSheet.textures['outro_cha.png']);
	...
	async onStart() {
		...
		// SoundManager.Handle.play('common', 'intro_bgm.mp3');
	...
	private async preLoadSound() {
		...
		// }
		const tSnds = [];
		// tSnds.push([Rsc.viewer, this.name, 'snd_bgm.mp3', true, true]);
