...
class StrokeCircle extends PIXI.Container {
	...
	constructor() {
		...
		this.mCircle.beginFill(0x000000);
		this.mCircle.drawCircle(0, 0, 53);
		this.mCircle.endFill();
		this.mCircle.position.set(-5, -5);
...
class Alphabet extends ObjectBase {
	...
	constructor(public m_Head: string, public m_Category: string) {
		...
		// 	this.m_Category,
		// 	`line_${this.m_Head}_1.png`,
		// ).texture;
		const lineSp = this.mImgSheet.textures[`line_${this.m_Head}_1.png`];
		...
		this.on('pointermove', (evt: PIXI.InteractionEvent) => {
			...
			if (this.mStartTrue && this.mCnt < this.mPos.length) {
				...
				if (dis < 40 && !this.mPos[this.mCnt].flag) {
		...
		// 	ViewerRscManager.Handle.getResource('common', 'finger.png').texture,
		// );
	...
	private createPointGuide() {
		...
		for (let i = 0; i < this.mMaxStoke; i++) {
			...
			// 	ProductRscManager.Handle.getResource(
			// 		this.m_Category,
			// 		`point_${this.m_Head}_1_${i + 1}.png`,
			// 	).texture,
			// );
			const tPointGuideSp = new PIXI.Sprite(
				this.mImgSheet.textures[`point_${this.m_Head}_1_${i + 1}.png`],
			);
	...
	private creatStrokeAry() {
		...
		for (let i = 0; i < this.mMaxStoke; i++) {
			...
			const ts = new TracingStoke(
				// ProductRscManager.Handle.getResource(
				// 	this.m_Category,
				// 	`${this.m_Head}_1_${i + 1}.png`,
				// ).texture,
				this.mImgSheet.textures[`${this.m_Head}_1_${i + 1}.png`],
				this.mTracingColor,
			);
			this.mStrokeAry.push(ts);
			ts.visible = false;
	...
	async makeStroke() {
		...
		if (this.mCurrenStoke < this.mMaxStoke) {
			...
			const tAlphabetSnd = SoundManager.Handle.getSound(
				'common',
				`snd_${App.Handle.getAlphabet}_big.mp3`,
			);
			tAlphabetSnd.play();
...
export class BigLetter extends SceneBase {
	...
	async onInit() {
		...
		// 	ViewerRscManager.Handle.getResource('common', 'edge.png').texture,
		// 	50,
		// 	50,
		// 	50,
		// 	50,
		...
		// const tNoteImg = new PIXI.Sprite(
		// 	ViewerRscManager.Handle.getResource('common', 'bignote.png').texture,
		// );
		...
		this.mAlphabetSpine = new PIXI.spine.Spine(
			ProductRscManager.Handle.getResource(
				this.name,
				`${App.Handle.getAlphabet}_1_tracing.json`,
			).spineData,
		);
		...
		// const tPalletSp = new PIXI.Sprite(
		// 	ViewerRscManager.Handle.getResource('common', 'pallet.png').texture,
		// );
		...
		// 	ViewerRscManager.Handle.getResource('common', 'redbtn_ori.png').texture,
		// 	ViewerRscManager.Handle.getResource('common', 'redbtn_sel.png').texture,
		// );
		const tRedBtn = new Button(
		...
		// 	ViewerRscManager.Handle.getResource('common', 'bluebtn_ori.png').texture,
		// 	ViewerRscManager.Handle.getResource('common', 'bluebtn_sel.png').texture,
		// );
		const tBlueBtn = new Button(
