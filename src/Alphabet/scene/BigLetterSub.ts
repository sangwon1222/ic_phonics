...
class Alphabet extends ObjectBase {
	...
	constructor(public m_Head: string, public m_Category: string) {
		...
		const lineSp = this.mImgSheet.textures[`line_${this.m_Head}_3.png`];
		...
		this.mFillAlphabetSp = new PIXI.Sprite(
			this.mImgSheet.textures[`${this.m_Head}_3.png`],
		);
	...
	private createPointGuide() {
		...
		for (let i = 0; i < this.mMaxStoke; i++) {
			...
			// );
			const tPointGuideSp = new PIXI.Sprite(
				this.mImgSheet.textures[`point_${this.m_Head}_3_${i + 1}.png`],
			);
	...
	private creatStrokeAry() {
		...
		for (let i = 0; i < this.mMaxStoke; i++) {
			...
			const ts = new TracingStoke(
				...
				// ).texture,
				this.mImgSheet.textures[`${this.m_Head}_3_${i + 1}.png`],
				this.mTracingColor,
			);
...
export class BigLetterSub extends SceneBase {
	...
	async onInit() {
		...
		// 	ViewerRscManager.Handle.getResource('common', 'starbg.png').texture,
		// );
		...
		// 	ViewerRscManager.Handle.getResource('common', 'card_empty.png').texture,
		// );
		this.mEmptyCardSp = new PIXI.Sprite(
		...
		// 	ViewerRscManager.Handle.getResource('common', 'question.png').texture,
		// );
		this.mWordCardSp = new PIXI.Sprite(
	...
	private setQuest() {
		...
		console.log(
			`${this.mStudyWords}, ${tQuestNum}, ${this.mStudyWords[tQuestNum]}`,
		);
		...
		if (this.mStudyWordTxt == 'box' || this.mStudyWordTxt == 'fox') {
			...
			this.mAlphabet.position.set(
				this.mWordTxtSp.x + this.mWordTxtSp.width / 2 - 100 + tPosGap[0][0],
				339 + tPosGap[0][1],
			);
	...
	private async showCorrectQuest() {
		...
		// await SoundManager.Handle.loadPreSound(
		// 	Rsc.product,
		// 	'common',
		// 	`${this.mStudyWordTxt}.mp3`,
		// );
		...
		// this.mWordCardSp.texture = ProductRscManager.Handle.getResource(
		// 	'common',
		// 	`${this.mStudyWordTxt}.png`,
		// ).texture;
		...
		if (this.mScoreCnt < this.mFinStarCnt) {
			...
			// this.mWordCardSp.texture = ViewerRscManager.Handle.getResource(
			// 	'common',
			// 	'question.png',
			// ).texture;
			...
			// gsap.to(this.mWordCardSp, { alpha: 1, duration: 0.5 });
			// gsap.to(this.mWordTxtSp, { alpha: 1, duration: 0.5 });
			// gsap.to(this.mAlphabet, { alpha: 1, duration: 0.5 });
		} else {
			this.clearActivity();
