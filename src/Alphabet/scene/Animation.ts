...
export class Animation extends SceneBase {
	...
	async onInit() {
		...
		this.mvideoController = new VideoPlayer(
			ProductRscManager.Handle.getResource(
				this.name,
				`${App.Handle.getAlphabet}_ani.mp4`,
			).data,
			0,
			32,
		);
		...
		this.mAniTrueImg = new PIXI.Sprite(
			this.mViewSheet.textures['truetrue_ufo.png'],
		);

		// this.mAniTrueImg = new PIXI.Sprite(
		// 	ViewerRscManager.Handle.getResource(this.name, 'truetrue.png').texture,
		// );
	...
	private async preLoadSound() {
		...
		tSnds.push([Rsc.product, this.name, `snd_${App.Handle.getAlphabet}_1.mp3`]);
	...
	private async showTrueTrue() {
		...
		const tSndName = `snd_${App.Handle.getAlphabet}_1.mp3`;
		const tReadySnd = SoundManager.Handle.getSound(this.name, tSndName);
