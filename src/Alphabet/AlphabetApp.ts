...
import { BigLetter } from './scene/BigLetter';
import { BigLetterSub } from './scene/BigLetterSub';
import { SmallLetter } from './scene/SmallLetter';
import { SmallLetterSub } from './scene/SmallLetterSub';
...
export class AlphabetApp extends App {
	...
	constructor(canvas: HTMLCanvasElement) {
		console.log('IScream Alphabet');
		Config.appName = 'alphabet';
		super(canvas);
		...
		this.addScene([new Animation()]); //1
		// // this.addScene([new BigLetterSub()]); //2, 3
		this.addScene([new BigLetter(), new BigLetterSub()]); //2, 3
		// // this.addScene([new SmallLetterSub()]); //4, 5
		this.addScene([new SmallLetter(), new SmallLetterSub()]); //4, 5
		this.addScene([new Game()]); //6
		this.addScene([new Outro()]); //7
	...
	private async startApp() {
		...
		if (SceneNum.SelectScene !== 0) {
			...
			this.mLoadingBg.beginFill(
				Number(AppConf.LoadingColor[Config.subjectNum - 1]),
			);
	...
	private setAssignableViewed() {
		...
		switch (tSceneNum) {
			case SceneNum.Animation:
				tSceneName = 'Animation';
				break;
			case SceneNum.BigLetter:
			case SceneNum.BigLetterSub:
				tSceneName = 'BIG LETTER';
				break;
			case SceneNum.SmallLetter:
			case SceneNum.SmallLetterSub:
				tSceneName = 'small letter';
				break;
			case SceneNum.Game:
				tSceneName = 'Game';
				break;
	...
	private activeBottomBar(tClear: boolean) {
		...
		switch (tSceneIdx) {
			...
			case SceneNum.SmallLetter:
			case SceneNum.BigLetter: {
				this.mBottomBar.settingButton(NaviBtnName.Sub1, BtnState.Selected);
				if (tSceneIdx < tLockIdx) {
					this.mBottomBar.settingButton(NaviBtnName.Sub2, BtnState.On);
					if (tClear) {
						this.mBottomBar.settingButton(NaviBtnName.Sub1, BtnState.Clear);
						this.mBottomBar.settingButton(NaviBtnName.Sub2, BtnState.Clear);
					}
				} else {
					this.mBottomBar.settingButton(NaviBtnName.Sub2, BtnState.Off);
				}
			...
			case SceneNum.SmallLetterSub:
			case SceneNum.BigLetterSub: {
				this.mBottomBar.settingButton(NaviBtnName.Sub1, BtnState.On);
				this.mBottomBar.settingButton(NaviBtnName.Sub2, BtnState.Selected);
	...
	private receiveMoveScene(evt: CustomEvent) {
		...
		switch (way) {
			...
			case NaviBtnName.Sub1:
				if (
					tSceneIdx === SceneNum.BigLetterSub ||
					tSceneIdx === SceneNum.SmallLetterSub
				) {
					this.prevScene();
				}
				break;
			case NaviBtnName.Sub2:
				if (
					tSceneIdx === SceneNum.BigLetter ||
					tSceneIdx === SceneNum.SmallLetter
				) {
					this.nextScene();
				}
				break;
