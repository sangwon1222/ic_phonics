...
export class Animation extends SceneBase {
	...
	async onInit() {
		...
		this.mAniTrueImg = new PIXI.Sprite(
			ViewerRscManager.Handle.getResource('common', 'truetrueUFO.png').texture,
		);
	...
	private async preLoadSound() {
		...
		tSnds.push([Rsc.product, this.name, `pr_an_${Config.subjectNum}.mp3`]);
	...
	private async showTrueTrue() {
		...
		const tSndName = `pr_an_${Config.subjectNum}.mp3`;
