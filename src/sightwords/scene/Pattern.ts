...
export class Pattern extends SceneBase {
	...
	private async preLoadSound() {
		...
		tSnds.push([Rsc.viewer, this.name, `sw_pa_dic.mp3`]);
	...
	private async showTrueTrue() {
		...
		const tReadySnd = SoundManager.Handle.getSound(this.name, `sw_pa_dic.mp3`);
