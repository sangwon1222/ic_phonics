...
import { ActivityOne } from './scene/ActivityOne';
import { ActivityTwo } from './scene/ActivityTwo';
...
export class PhonicsApp extends App {
	...
	constructor(canvas: HTMLCanvasElement) {
		console.log('IScream Phonics Reading');
		Config.appName = 'ph_reading';
		super(canvas);
		...
		this.addScene([new Words()]); //1
		this.addScene([new Animation()]); //2
		this.addScene([new Chant()]); //3
		// this.addScene([new ActivityTwo()]); //4,5
		this.addScene([new ActivityOne(), new ActivityTwo()]); //4,5
	...
	private async startApp() {
		...
		if (SceneNum.SelectScene !== 0) {
			...
			this.mLoadingBg.beginFill(0x48bfb0);
	...
	private setAssignableViewed() {
		...
		switch (tSceneNum) {
			...
			case SceneNum.Chant:
				tSceneName = 'Chant';
				break;
			case SceneNum.ActivityOne:
			case SceneNum.ActivityTwo:
				tSceneName = 'Activity';
				break;
			default:
				tSceneName = '';
	...
	private receiveMoveScene(evt: CustomEvent) {
		...
		switch (way) {
			...
			case NaviBtnName.Sub1:
				if (tSceneIdx === SceneNum.ActivityTwo) {
					this.prevScene();
				}
				break;
			case NaviBtnName.Sub2:
				if (tSceneIdx === SceneNum.ActivityOne) {
					this.nextScene();
				}
				break;
			default:
